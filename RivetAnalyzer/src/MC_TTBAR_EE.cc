...
namespace Rivet {
  ...
  class MC_TTBAR_EE : public Analysis {

  public:

    MC_TTBAR_EE()
      : Analysis("MC_TTBAR_EE")
    {
      _sumwPassedLep = 0;
      _sumwPassedMET = 0;
      _sumwPassedJet = 0;
    ...
    void analyze(const Event& event) {
      ...
      if ( sortedElectrons.size() < 2) {
        MSG_DEBUG("Event failed muon multiplicity cut");
        vetoEvent;
      }

      int chlep1 = PID::threeCharge(sortedElectrons[0]) ;
      _h_chleps->fill(chlep1, weight);
      int chlep2 = PID::threeCharge(sortedElectrons[1]) ;
      ...
      _h_nleps->fill(sortedElectrons.size(), weight);
      _h_lep_1_pT->fill(sortedElectrons[0].momentum().pT(), weight);
      _h_lep_2_pT->fill(sortedElectrons[1].momentum().pT(), weight);
      ...
      const FourMomentum ttbar = bjets[0].momentum() + bjets[1].momentum() + sortedElectrons[0].momentum() + sortedElectrons[1].momentum() + missvec ;
  ...
  AnalysisBuilder<MC_TTBAR_EE> plugin_MC_TTBAR_EE;

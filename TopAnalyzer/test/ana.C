...
void ana()
{
  ana("MuMu", "Testresult06Jun/MuMu");
  ana("ElEl", "Testresult06Jun/ElEl");
  ana("MuEl", "Testresult06Jun/MuEl");
}
...
void ana(string decayMode, string imageOutDir)
{
  ...
  const std::string mcPath = "$WORK/data/export/common/Top/ntuple/"+decayMode+"/MC/Fall11_v6/";
  const std::string rdPath = "$WORK/data/export/common/Top/ntuple/"+decayMode+"/RD/2011Full_v4/";

  analyzer->addRealData(rdPath+"vallot.root", 4982);

  analyzer->addMCSig("TTbar", "t#bar{t}", mcPath+"vallot_TTbarTuneZ2.root", 161.9, kRed+1);
  analyzer->addMCBkg("TTbarOthers", "t#bar{t} other", mcPath+"vallot_TTbarOthers.root", 161.9, kRed-7);

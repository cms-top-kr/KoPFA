...
void anal(bool p= false, const TString& d="temp"){
  print = p;
  dir = d;
  ...
  using namespace std;
  //fdata = new TFile("/home/tjkim/ntuple/data/top/Aug13/vallot_presentedAug26.root");
  //fdata = new TFile("/home/tjkim/ntuple/data/top/Aug13/vallot.root");
  fdata = new TFile("/home/tjkim/ntuple/data/top/Sep3_noPileupSequence/vallot.root");
  ...
  TFile * fVVJets = new TFile("/home/tjkim/ntuple/mc/top/Spring10/vallot_VVJets.root");
  TFile * fSingleTop = new TFile("/home/tjkim/ntuple/mc/top/Spring10/vallot_SingleTop.root");

  data = (TTree *) fdata->Get("DiMuon/tree");

  addFile(fTTbar, "TTbar", "t#bar{t}", 157.5, 4);//Blue
  addFile(fQCD, "QCD", "QCD", 79688, 3);//Green
  addFile(fWmunu, "Wlnu", "W #rightarrow l#nu", 31314, 46);// brown
  addFile(fVVJets, "VV", "VV", 4.8 ,6 );//Magenta
  addFile(fSingleTop, "SingleTop", "single top", 10.6 ,7 );// sky
  addFile(fZtautau, "Ztautau", "Z/#gamma* #rightarrow #tau#tau", 1660, 5);//Yellow
  addFile(fZmumu, "Zll", "Z/#gamma* #rightarrow ll", 3048, 2);//Red  

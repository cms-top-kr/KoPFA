#ifndef KoPFA_ElectronAnalysis_PFElectronAnalyzer_H
#define KoPFA_ElectronAnalysis_PFElectronAnalyzer_H
...
{
public:
  PFElectronAnalyzer(const edm::ParameterSet& pset);
  ~PFElectronAnalyzer();
...
PFElectronAnalyzer::PFElectronAnalyzer(const edm::ParameterSet& pset)
{
...
void PFElectronAnalyzer::beginJob()
{
  ...
  h_gsf_mva = fs->make<TH1F>( "h_gsf_mva","h_gsf_mva", 140, -6,1);

  h_pf_pt = fs->make<TH1F>( "h_pf_pt","h_pf_pt", 50, 0,100);
  h_pf_eta = fs->make<TH1F>( "h_pf_eta","h_pf_eta", 70, -3.5,3.5);
  h_pf_mva = fs->make<TH1F>( "h_pf_mva","h_pf_mva", 140, -6,1);
  h_pf_ecal = fs->make<TH1F>( "h_pf_ecal","h_pf_ecal", 300, 0, 3);
  h_pf_gsfecal = fs->make<TH1F>( "h_pf_gsfecal","h_pf_gsfecal", 300, 0, 3);
  
}

void PFElectronAnalyzer::endJob()
{

}

PFElectronAnalyzer::PFCandIter PFElectronAnalyzer::findElectron(const GsfEleIter gsfEle, 
                                                            PFElectronAnalyzer::PFCandIter begin, 
                                                            PFElectronAnalyzer::PFCandIter end)
{
...
PFElectronAnalyzer::GsfEleIter PFElectronAnalyzer::findElectron(const PFCandIter pfaEle, 
                                                            PFElectronAnalyzer::GsfEleIter begin, 
                                                            PFElectronAnalyzer::GsfEleIter end)
{
...
void PFElectronAnalyzer::analyze(const edm::Event& event, const edm::EventSetup& eventSetup)
{
  using namespace std;
  using namespace edm;
      ...
      iPFCand != pfCandHandle->end(); ++iPFCand )
  {
    ...
    if( iPFCand->mva_e_pi() < -0.1){
      h_pf_ecal->Fill(iPFCand->ecalEnergy()/iPFCand->p());
      h_pf_gsfecal->Fill(matched->ecalEnergy()/matched->p());
    }

...
process.out = cms.OutputModule("PoolOutputModule",
    AODEventContent,    
     fileName = cms.untracked.string('PF2PAT_SDMUON_GOODCOLL.root'),
     )
...
process.load("PFAnalyses.W.wMuNu_cff")
process.ipMuons.src =  cms.InputTag("selectedPatMuonsPFlow")

#process.wMuNuCandsMuPt.cut = cms.string('visDecayProducts().pt()>5.')
#process.wMuNuCandsMuEta.cut = cms.string('abs(visDecayProducts().eta())<2.4')
...
process.muonFilter = cms.Path(process.primaryVertexFilter
                             +process.noscraping
#                             +process.Mu_1e28
                             +process.patMuonSelector
...
SelectEvents = cms.vstring('goodvertex') #When running on SD no need to have a further L1 requirement
)
...
process.DiMuon = cms.Path(process.DiMuonPFlow)

...
namespace Rivet {
  ...
  class MC_TTBAR_MM : public Analysis {

  public:

    MC_TTBAR_MM()
      : Analysis("MC_TTBAR_MM")
    {
      _sumwPassedLep = 0;
      _sumwPassedMET = 0;
      _sumwPassedJet = 0;
    ...
    void analyze(const Event& event) {
      ...
      int chlep2 = PID::threeCharge(sortedMuons[1]) ;
      ...
      _h_lep_2_pT->fill(sortedMuons[1].momentum().pT(), weight);
      ...
      const FourMomentum ttbar = bjets[0].momentum() + bjets[1].momentum() + sortedMuons[0].momentum() + sortedMuons[1].momentum() + missvec ;
  ...
  AnalysisBuilder<MC_TTBAR_MM> plugin_MC_TTBAR_MM;

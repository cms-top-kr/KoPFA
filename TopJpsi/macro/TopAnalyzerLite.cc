...
TopAnalyzerLite::TopAnalyzerLite(const string subDirName, const string imageOutDir, bool createplots, bool printstats)
{
  ...
  scanVariables_ = "RUN:LUMI:EVENT:lep1_pt:lep2_pt:JPsiPt:JPsiMass:LepJPsiMass1:LepJPsiMass2";
  //scanVariables_ = "RUN:LUMI:EVENT:lep1_pt:lep2_pt:jets_pt:JPsiPt";
  eventWeightVar_ = "";
...
void TopAnalyzerLite::saveHistograms(TString fileName)
{
  ...
  for ( unsigned int i=0; i<dirNames.size(); ++i )
  {
    ...
    if(realDataChain_) {
       //realDataChain_->SetEntryList(entryList_["realdata"].at(i));
       TString tName = Form("tree_%s",dirName.Data());
       if( tName.Contains( dirName.Data() )) {
         if(i>3) {
           cout<<"["<<i<<"] : "<<dirName<<"("<<entryList_["realdata"].at(i)->GetN()<<")"<<endl;
           Long64_t listEntries =entryList_["realdata"].at(i)->GetN();
           //Long64_t chainEntries = realDataChain_->GetEntries(); 
           Int_t treenum = 0;

           double JPsiMass, LepJPsiMass1, LepJPsiMass2;
           realDataChain_->SetBranchAddress("JPsiMass",&JPsiMass);
           realDataChain_->SetBranchAddress("LepJPsiMass1",&LepJPsiMass1);
           realDataChain_->SetBranchAddress("LepJPsiMass2",&LepJPsiMass2);

           TTree* t = new TTree(tName,tName);
           double jpsimass, ljpsimass1, ljpsimass2;
           t->Branch("jpsimass",&jpsimass,"jpsimass/D");
           t->Branch("ljpsimass1",&ljpsimass1,"ljpsimass1/D");
           t->Branch("ljpsimass2",&ljpsimass2,"ljpsimass2/D");

           realDataChain_->SetEntryList(entryList_["realdata"].at(i));
           //realDataChain_->Scan("EVENT:JPsiMass:LepJPsiMass1:LepJPsiMass2");

           for(Long64_t el = 0; el < listEntries; el++){
              Long64_t treeEntry = entryList_["realdata"].at(i)->GetEntryAndTree(el,treenum);
              Long64_t chainEntry = treeEntry+realDataChain_->GetTreeOffset()[treenum];
              //printf("el=%lld, treeEntry=%lld, chainEntry=%lld, treenum=%d\n", el, treeEntry, chainEntry, treenum);
              realDataChain_->GetEntry(treeEntry);
              jpsimass = JPsiMass;
              ljpsimass1 = LepJPsiMass1;
              ljpsimass2 = LepJPsiMass2;
              //cout<<"jpsimass : "<<jpsimass<<" , ljpsimass1 : "<<ljpsimass1<<" , ljpsimass2 : "<<ljpsimass2<<endl; 
              t->Fill();
